...
<head>
  ...
  <style>body{background:}.container{border-radius:5px;padding:20px;border-image:linear-gradient(100deg,#BF2727,#43cea2);border-top:25px solid rgba(214,238,198.5);box-shadow:0 0 50px}input[type=T2],select,textarea{width:100%;padding:12px;border:1px solid #ccc;border-radius:4px;box-sizing:border-box;margin-top:6px;margin-bottom:16px;resize:vertical;outline:none}</style>
</head>
<body>
	<?php if (isAllow()): ?>
		<!-- If eligible for show the download button -->

    <div class="container">

    <textarea rows="16" id="copy_textarea" name="TA1" spellcheck="false" ><?php
    $name = $_POST["myname"];
    $url = $_POST["myurl"];
    $mail = $_POST["mymail"];
    ...
    <center>
    <p><button onclick="myFunction()" class="btn btn-outline-primary btn-lg" type="button" id="copy_button">Copy Text to Clipboard<span id="copy_tooltip" aria-live="assertive" role="tooltip"></span></button></p></center>
    </div>
	...
	<?php else: ?>
		<!-- If NOT eligible for show the download button than -->
		<!-- You can show hare onether content OR keep blank! -->
...
<?php
/* At first make allowd linkable function */
function isAllow() {
	$security = array(

	/* Set allowed links */
	'https://kitbox.org/privacy-policy-generator/',
	'http://localhost:4000/sec/index.php'
	);

	if (isset($_SERVER['HTTP_REFERER']) AND in_array($_SERVER['HTTP_REFERER'], $security)) {
		return true;
		/* return true; so that this function automatic execute */
	}
